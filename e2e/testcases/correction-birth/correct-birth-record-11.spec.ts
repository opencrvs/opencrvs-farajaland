...
test.describe('11. Correct record', () => {
  ...
  test.describe.serial('11.1 Record correction by someone else', async () => {
    let page: Page
    ...
    test('11.0.0 Navigate to record correction', async () => {
    ...
    test('11.1.0 Correction details', async () => {
    ...
    test('11.1.1 Verify identity', async () => {
    ...
    test('11.1.2 Upload supporting documents', async () => {
    ...
    const fee = faker.number.int({ min: 1, max: 1111 })

    test('11.1.3 Fees', async () => {
      await expectInUrl(
        page,
    ...
    test.describe('11.1.4 Correction made on child details', async () => {
      test('11.1.4.1 Change name', async () => {
      ...
      test('11.1.4.2 Change gender', async () => {
      ...
      test('11.1.4.3 Change date of birth', async () => {
      ...
      test('11.1.4.4 Change place of delivery', async () => {
        ...
        // let location = await page.locator('//input[@id="district"]')')

        // await page
        //   .locator('//input[@id="district"]')

        //   .fill(updatedChildDetails.birthLocation.slice(0, 2))
        //   .waitForSelector('.react-select__menu')
        // await page.getByText(updatedChildDetails.birthLocation).click()

        const district = page.locator('#district')

        await district.click()
        await district.fill(updatedChildDetails.birthLocation.slice(0, 2))

        await page.waitForSelector('.react-select__menu')

        await page
          .locator('.react-select__menu')
          .getByText(updatedChildDetails.birthLocation, { exact: true })
          .click()
        ...
        await expect(
          page
            .getByTestId('row-value-child.birthLocation.privateHome')
            .getByRole('deletion')
      ...
      test('11.1.4.5 Change attendant at birth', async () => {
      ...
      test('11.1.4.6 Change type of birth', async () => {
    ...
    test.describe('11.1.5 Correction summary', async () => {
      test('11.1.5.1 Go back to review', async () => {
        await page
      ...
      test('11.1.5.2 Change weight at birth', async () => {
      ...
      test('11.1.5.3 Validate information in correction summary page', async () => {
        await page
          .getByRole('button', { name: 'Continue', exact: true })
          .click()
        ...
        await expect(
          page.getByText(
            'Location of birthFarajalandCentralIbomboDhakaFarajalandCentralItamboDhaka'
          )
        ).toBeVisible()
    ...
    test.describe('11.1.6 Correction Approval', async () => {
      test.beforeAll(async ({ browser }) => {
        await page.close()
        page = await browser.newPage()
      ...
      test('11.1.6.1 Record audit by local registrar', async () => {
        if (!trackingId) {
      ...
      test('11.1.6.2 Correction review', async () => {
      ...
      test('11.1.6.3 Approve correction', async () => {
        await page.getByRole('button', { name: 'Approve', exact: true }).click()
        await page.getByRole('button', { name: 'Confirm', exact: true }).click()

        await expectInUrl(page, `/events/${eventId}`)
      })
      test.describe('11.1.6.4 Validate history in record audit', async () => {
        test('11.1.6.4.1 Validate correction requested modal', async () => {
          await ensureAssigned(page)
        ...
        test('11.1.6.4.2 Validate correction approved modal', async () => {
