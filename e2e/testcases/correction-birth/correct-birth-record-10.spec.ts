...
test('10. Correct record', async ({ browser }) => {
  ...
  await test.step('10.0.0 Navigate to record correction', async () => {
  ...
  await test.step('10.1.0 Correction details', async () => {
  ...
  await test.step('10.1.1 Verify identity', async () => {
  ...
  await test.step('10.1.2 Upload supporting documents', async () => {
  ...
  await test.step('10.1.3 Fees', async () => {
    await expectInUrl(
      page,
  ...
  await test.step('10.1.4.1 Change name', async () => {
  ...
  await test.step('10.1.4.2 Change gender', async () => {
  ...
  await test.step('10.1.4.3 Change date of birth', async () => {
  ...
  await test.step('10.1.4.4 Change place of delivery', async () => {
  ...
  await test.step('10.1.4.5 Change attendant at birth', async () => {
  ...
  await test.step('10.1.4.6 Change type of birth', async () => {
  ...
  await test.step('10.1.5.2 Change weight at birth', async () => {
  ...
  await test.step('10.1.6 Correction Approval', async () => {
    await page.close()
  })

  const page2 = await browser.newPage()

  await test.step('10.1.6 Login as Registrar', async () => {
    await login(page2, CREDENTIALS.REGISTRAR)
  })

  await test.step('10.1.6.1 Record audit by Registrar', async () => {
    if (!trackingId) {
      throw new Error('Tracking ID is required')
    }

    await type(page2, '#searchText', trackingId)
    await page2.locator('#searchIconButton').click()
    await page2
      .getByRole('button', { name: formatV2ChildName(declaration) })
      .click()
  })

  await test.step('10.1.6.2 Correction review', async () => {
    await selectAction(page2, 'Review correction request')
    ...
    await expect(page2.getByText('ID number' + nationalId)).toBeVisible()
    await expect(
      page2.getByText('Name' + `${name.firstname} ${name.surname}`)
    ).toBeVisible()
    await expect(
      page2.getByText('Relationship to child' + relationship)
    ).toBeVisible()
    await expect(
      page2.getByText('Reason for correction' + 'Other')
    ).toBeVisible()
    await expect(page2.getByText('Specify reason' + reason)).toBeVisible()
    await expect(page2.getByText('Fee total' + '$' + fee)).toBeVisible()
  })

  await test.step('10.1.6.3 Approve correction', async () => {
    await page2.getByRole('button', { name: 'Approve', exact: true }).click()
    await page2.getByRole('button', { name: 'Confirm', exact: true }).click()

    await expectInUrl(page2, `/events/${eventId}`)
  })

  await test.step('10.1.6.4.0 Ensure record is assigned', async () => {
    await ensureAssigned(page2)
  })

  await test.step('10.1.6.4.1 Validate correction requested modal', async () => {
    await page2.getByRole('button', { name: 'Audit' }).click()
    await page2
    ...
    await expect(page2.getByText('Type of birth-Twin')).toBeVisible()
    await page2.locator('#close-btn').click()
  })

  await test.step('10.1.6.4.2 Validate correction approved modal', async () => {
