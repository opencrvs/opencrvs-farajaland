...
test('1. Correct record - 1', async ({ page }) => {
  ...
  await test.step('Create a registered birth event via API', async () => {
  ...
  await test.step('1.1.1 Validate record audit page', async () => {
    ...
    const childAuditBirthLocationName = await getLocationNameFromId(
      birthLocationId,
      token
    )
    await expect(
      page.getByText(`Place of birth${childAuditBirthLocationName}`)
    ).toBeVisible()
    await expect(
  ...
  await test.step('1.1.2 Validate correction requester page', async () => {
  ...
  await test.step('1.1.3 Validate identity verification page for Mother', async () => {
  ...
  // 1.2 Record correction by informant (mother).
  ...
  // 1.2.4 Correction made on child details.
