...
test.describe.serial('1.1 Basic UI check', async () => {
  ...
  test('2.1.2 Verify Province-> Distric -> District Office(No Data)', async () => {
    for (let i = 0; i < 3; i++) {
      await page.goBack()
    }
    ...
    await page.getByRole('button', { name: /Funabuli District Office/ }).click()
    await expect(page.locator('#content-name')).toHaveText(
      /Funabuli District Office/
    )
    await expect(
  ...
  test('2.1.3 Verify team page member list', async () => {
