...
test.describe
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    ;[yyyy, mm, dd] = record.declaration['spouse.dob'].split('-')
  })
  ...
  test.describe.serial("2.5 - Validate search by Informant's details", () => {
    ...
    test('2.5.2 - Validate search and show results', async () => {
      ...
      await expect(page.url()).toContain(
        `informant.name=${encodeURIComponent(JSON.stringify({ firstname: record.declaration['spouse.name'].firstname, middlename: '', surname: record.declaration['spouse.name'].surname }))}`
      )
      ...
      await expect(
        page.getByText(
          `Informant's Date of birth: ${formatDateToLongString(record.declaration['spouse.dob'])}`
        )
      ).toBeVisible()
      await expect(
        page.getByText(
          `Informant's Name: ${record.declaration['spouse.name'].firstname} ${record.declaration['spouse.name'].surname}`
        )
      ).toBeVisible()
    ...
    test('2.5.3 - Validate clicking on the search edit button', async () => {
      ...
      await expect(page.url()).toContain(`informant.dob=${yyyy}-${mm}-${dd}`)
      await expect(page.url()).toContain(
        `informant.name=${encodeURIComponent(JSON.stringify({ firstname: record.declaration['spouse.name'].firstname, surname: record.declaration['spouse.name'].surname, middlename: '' }))}`
      )
