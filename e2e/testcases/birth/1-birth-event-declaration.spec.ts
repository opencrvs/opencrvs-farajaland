...
test('1. Birth event declaration - Fill all form sections. Save & Exit', async ({
  browser
}) => {
  const page = await browser.newPage()

  await test.step('1.1. Navigate to the birth event declaration page', async () => {
    await login(page)
  ...
  await test.step('1.5.1 Validate the contents of informant details page', async () => {
    /*
     * Expected result: should see
  ...
  await test.step('1.5.3 Select any option in relationship to child and click the "continue" button', async () => {
  ...
  // test.skip("1.6.2 Validate Mother's details block")
  ...
  // test.skip("1.7.2 Validate Father's details block")
  ...
  await test.step('1.9.1 Click the "Save & Exit" button from review page', async () => {
  ...
  await test.step('1.9.4 Reopen draft and navigate to review page', async () => {
    await page
      .getByRole('button', {
...
test('1.10 Validate "Exit" Button', async ({ page }) => {
  await login(page)
  await page.click('#header-new-event')
  await page.getByLabel('Birth').click()
  await page.getByRole('button', { name: 'Continue' }).click()
...
test('1.10.3 Click Confirm on Exit', async ({ page }) => {
  await login(page)
  await page.click('#header-new-event')
  await page.getByLabel('Birth').click()
  await page.getByRole('button', { name: 'Continue' }).click()
...
test('1.11 Validate "Delete Declaration" Button', async ({ page }) => {
  await login(page)
  await page.click('#header-new-event')
  await page.getByLabel('Birth').click()
  await page.getByRole('button', { name: 'Continue' }).click()
...
test('1.11.3 Click Confirm on Delete Declaration', async ({ page }) => {
  await login(page)
  await page.click('#header-new-event')
  await page.getByLabel('Birth').click()
  await page.getByRole('button', { name: 'Continue' }).click()
