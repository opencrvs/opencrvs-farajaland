...
docker_stack_deploy() {
  ...
  GENERATED_VARIABLES="ROTATING_METRICBEAT_ELASTIC_PASSWORD=$ROTATING_METRICBEAT_ELASTIC_PASSWORD
  ...
  configured_ssh 'cd /opt/opencrvs && \
    '$GENERATED_VARIABLES' docker stack deploy --prune -c '$(split_and_join " " " -c " "$COMMON_COMPOSE_FILES infrastructure/$environment_compose")' --with-registry-auth opencrvs'
}
