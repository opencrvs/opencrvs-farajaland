...
const AUTH_URL = 'http://localhost:4040'
...
const headers = {
    ...
    "cookie": "metabase.DEVICE=090fd868-3495-497f-b4d5-eafd2bff0c0b; metabase.TIMEOUT=alive; metabase.SESSION=dd3a3919-c4a4-4bd4-afb4-34e781875185",
    "Referer": "http://localhost:3000/workqueue/assigned-to-you"
}

for (let i = 0; i < 10000; i++) {


    const createResponse = await fetch("http://localhost:3000/api/events/event.create?batch=1", {
        headers,
        body: JSON.stringify({
            0: {
                json: {
    ...
    console.log(`${i + 1}: ${eventId} => ${applicantName.firstname} ${applicantName.surname}`);

    // console.log({ applicantName, eventId, transactionId });


    const declareResponse = await fetch("http://localhost:3000/api/events/event.actions.declare.request?batch=1", {
        headers,
        body: JSON.stringify({
            0: {
                json: {
    ...
    const validateResponse = await fetch("http://localhost:3000/api/events/event.actions.validate.request?batch=1", {
        headers,
        body: JSON.stringify({
            0: {
                json: {
    ...
    const registerResponse = await fetch("http://localhost:3000/api/events/event.actions.register.request?batch=1", {
        headers,
        "referrer": "http://localhost:3000/workqueue/assigned-to-you",
        body: JSON.stringify({
            0: {
                json: {
                    declaration: {},
